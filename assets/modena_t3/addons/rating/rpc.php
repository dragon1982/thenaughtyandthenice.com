...
{     //if the user hasn't yet voted, then vote normally...
			...
			$result = mysql_query("SELECT * FROM vote_log WHERE ip='".$ip."' ORDER BY id DESC LIMIT 3;");
			$nr = 0;
			while($tester = mysql_fetch_array($result))
			{
				if ($tester['vot'] == 1):
					$nr++;
				endif;
			...
			{
				//fuck off
				header("Location: http://www.dnl.ro/proiecte3/sp");
				exit;
			}
			else
			{
				mysql_query("INSERT INTO vote_log VALUES(null,'".$ip."','".$vote_sent."');");
			}
...
$new_back[] .= '</ul>';
$new_back[] .= '<p class="voted"><strong>'.@number_format($sum/$added,1).'</strong>'.' - '.$count.' '.$tense.'</p>';
//$new_back[] .= '<span class="thanks">Multumim!</span></p>';
