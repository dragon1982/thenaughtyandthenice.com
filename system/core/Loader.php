...
class CI_Loader {
	...
	{
		$this->_ci_view_path = array(APPEXPATH.'views/',APPPATH.'views/');
		$this->_ci_ob_level  = ob_get_level();
		$this->_ci_library_paths = array(APPPATH, BASEPATH, APP_DEFAULT_PATH, EXTPATH,APPEXPATH);
		$this->_ci_helper_paths = array(APPEXPATH, APPPATH, APP_DEFAULT_PATH, EXTPATH,BASEPATH);
		$this->_ci_model_paths = array(APPPATH, APP_DEFAULT_PATH, APPEXPATH);
	...
	*/
	...
	function model($model, $name = '', $db_conn = FALSE)
	{
		...
		foreach ($this->_ci_model_paths as $mod_path)
		{
			...
			if( file_exists(APPEXPATH . 'models/' . config_item('subclass_prefix') . $path . $model . EXT ) && $mod_path != APPEXPATH ){
				require_once APPEXPATH . 'models/' . config_item('subclass_prefix') . $path . $model . EXT;
				$model = config_item('subclass_prefix').ucfirst($model);
				$CI->$name = new $model();
	...
	function image($image_path){
		...
		$ext = strtolower(substr(strrchr($image_path, '.'), 1));
		
		$info = getimagesize($image_path);
    	$mime = image_type_to_mime_type($info[2]);
    	
		header('Content-Length: ' . filesize($image_path)); //<-- sends filesize header
		header('Content-Type: ' . $mime); //<-- send mime-type header
		//header('Content-Disposition: inline; filename="' . md5($image_path . 'thisrand' ) . '.'.  $ext . '";'); //<-- sends filename header
	...
	function helper($helpers = array())
	{
		foreach ($this->_ci_prep_filename($helpers, '_helper') as $helper)
		{
			...
			$found_ext = FALSE;
			$ext_helper = APPEXPATH.'helpers/'.$helper.EXT;
			$app_in_helper = APPPATH.'helpers/'.$helper.EXT;
			$app_helper = APP_DEFAULT_PATH.'helpers/'.$helper.EXT;
			$base_helper = BASEPATH.'helpers/'.$helper.EXT;
			// Is this a helper extension request?

			if (file_exists($app_in_helper))
			{
				include_once($app_in_helper);
				
				$this->_ci_helpers[$helper] = TRUE;
				log_message('debug', 'Helper loaded: '.$helper);
				$found_ext = TRUE;				
			} elseif(file_exists($ext_helper) ){
				include_once($ext_helper);
				
				$this->_ci_helpers[$helper] = TRUE;
				log_message('debug', 'Helper loaded: '.$helper);
				$found_ext = TRUE;				
			} elseif(file_exists($app_helper)) {
	...
	function _ci_load($_ci_data)
	{
		...
		if ($_ci_path == '')
		{
			...
			$_ci_path = (file_exists($this->_ci_view_path[0].$_ci_file))?$this->_ci_view_path[0].$_ci_file:$this->_ci_view_path[1].$_ci_file;
		}
		else
		{
	...
	function _ci_load_class($class, $params = NULL, $object_name = NULL)
	{
		...
		foreach (array(ucfirst($class), strtolower($class)) as $class)
		{
			...
			$subclass = APP_DEFAULT_PATH.'libraries/'.$subdir.config_item('subclass_prefix').$class.EXT;
