...
class CI_DB_active_record extends CI_DB_driver {
	...
	private $union_order		= FALSE;
	
	var $_memcache_option		= FALSE;
	var $_memcache_save_result 	= FALSE;
	var $_memcache_cache_duration = 0;

	// --------------------------------------------------------------------
	/**
	 * Seteaza keia din memcache
	 * 
	 * @param $key
	 * @param $args
	 * @param $duration in case key does not exist how many seccons to keep it in cache
	 * @return unknown_type
	 */
	function set_memcache_key($key,$args,$duration){
		if( ! MEMCACHE_ENABLE )
		...
		//sanitize the params
		$this->sanitize_memcache_params( $args);

		$this->_memcache_cache_duration = $duration;
		
		if( ! is_array( $args ) || sizeof($args) == 0)
		{//argumentele nu sunt array, sau arrayul e gol
			
			$this->_memcache_option = $key;			
			return $this;
		}
			
		try
		{//construiesc keia de memcache
			$this->_memcache_option = $this->sprintf_array($key,$args);	
		}
		catch(Exception $e){}
	...
	/**
	 * Sterge keia din cache
	 * @param $key
	 * @param $args
	 * @return object
	 */
	function drop_memcache_key($key,$args = array()){
		if( ! MEMCACHE_ENABLE )
		{ //memcache is not enabled			
			return $this;
		}
		...
		$result = $CI->cache->memcache->delete($this->_memcache_option);		
		
		return $this;								
	}
	
	/**
	 * Sterge eventualele caractere nedorite din siru de kei
	 * @param $args
	 * @return unknown_type
	 */
	function sanitize_memcache_params(& $args){
		if(sizeof($args) == 0){
			return FALSE;
		}
		foreach($args as $row => $arg){
			 $replaced = preg_replace("/[^a-zA-Z0-9\s]/", "", $arg);
			 if($replaced){
			 	$args[$row] = $replaced;
			 } else {
			 	unset($args[$row]);
	...
	/**
	 * Sprintf pe un array de caractere
	 * @param $format
	 * @param $args
	 * @return unknown_type
	 */
	function sprintf_array($format,$args){
	...
	/**
	 * Add braket support for active rekord:D
	 * @param $type
	 * @return unknown_type
	 */
	...
	function _where($key, $value = NULL, $type = 'AND ', $escape = NULL)
	{
		...
		foreach ($key as $k => $v)
		{
			...
			if( $this->add_symbol ){						
				$prefix .= $this->add_symbol;					
				$this->add_symbol = NULL;			
	...
	/**
	 * Creates an union with another table
	 * @param string $table
	 */
	...
	function get($table = '', $limit = null, $offset = null)
	{
		...
		if( $this->_union ){
			$sql = '(' . $sql;
			$table = str_replace('`','',current($this->ar_from));
			$string = str_replace($table, "{$this->_union}", $sql). ')';
			$sql .= ') UNION ' .  $string;
			
			if( $this->union_order ){
				$sql .= ' ' . str_replace(current($this->ar_from).'.','',$this->union_order). '
				 ';
			}
			
			if ( $this->ar_limit)
			{
				$concat = ' LIMIT ' . $this->ar_limit; 					 
	...
	function _compile_select($select_override = FALSE)
	{
		...
		if (count($this->ar_orderby) > 0)
		{
			...
			if(  $this->_union){			
				$this->union_order = "\nORDER BY ";
				$this->union_order .= implode(', ', $this->ar_orderby);
			
				if ($this->ar_order !== FALSE)
				{
					$this->union_order .= ($this->ar_order == 'desc') ? ' DESC' : ' ASC';
				}			
			}
		}
	...
	function _reset_select()
	{
		$ar_reset_items = array(
								...
								'_union'			=> FALSE,
								'union_order'		=> FALSE,
								'ar_bracket_open'	=> FALSE,
								'last_bracket_type'	=> 'where',
								'add_symbol'		=> NULL,
								
								'_memcache_option'			=> FALSE,
								'_memcache_save_result' 	=> FALSE,
								'_memcache_cache_duration' 	=> 0							
							);
