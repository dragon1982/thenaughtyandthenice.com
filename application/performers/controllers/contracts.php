 ...
 */
class Contracts_controller extends MY_Performer{
	 ...
	 */
	function index(){
		$data['contracts']				= $this->contracts->get_multiple_by_performer_id($this->user->id);
        $data['description'] 	       = SETTINGS_SITE_DESCRIPTION;
        $data['keywords'] 		       = SETTINGS_SITE_KEYWORDS;
        $data['pageTitle'] 		       = lang('My contracts').' - '.SETTINGS_SITE_TITLE;
	 ...
	 */
	function add(){		
		$approved_contracts = $this->contracts->get_multiple_by_performer_id($this->user->id,array('status'=>'approved'),TRUE);			
		...
		$total_contracts = $this->contracts->get_multiple_by_performer_id($this->user->id,FALSE,TRUE);
		...
		if( $this->form_validation->run() === FALSE ){				
	        ...
	        $data['description'] 	       = SETTINGS_SITE_DESCRIPTION;
	        $data['keywords'] 		       = SETTINGS_SITE_KEYWORDS;
	        $data['pageTitle'] 		       = lang('Add contract').' - '.SETTINGS_SITE_TITLE;
			$data['_performer_menu']		= TRUE;
        	$data['_sidebar']		       = TRUE;
			...
			foreach( $this->good_contracts as $contract ){	
				$name = generate_unique_name('uploads/performers/' . $this->user->id . '/others/',$contract['name']);
				
				//move the contract to his place
				@rename(MY_TEMP_PATH.'/' . $contract['file_on_disk_name'],'uploads/performers/' . $this->user->id . '/others/' . $name);
	...
	function delete( $contract_id = FALSE ){
		...
		if( $contract->performer_id !== $this->user->id ){
		...
		@unlink('uploads/performers/' . $this->user->id . '/others/' . $contract->name_on_disk);		
