...
function previewOpenVideoInModal(video_id){
	$.fancybox({
		...
		'href': '<?php echo site_url('performer/videos/view')?>' + '/' + video_id,
		'overlayColor': "#000",
		'overlayOpacity': "0.3"				
	});		
}
	...
	<div class="content">
		...
		<div id="videos">
			<?php if(sizeof($videos) == 0):?>
				<?php echo lang('There are no videos!')?>
			<?php else:?>
					...
					<div class="video">						
						<script type="text/javascript">		
							var flashvars_<?php echo $row?> = {
								...
								performerId			: '<?php echo $this->performer->id?>',
								videoId				: '<?php echo $video->video_id?>',								
								password			: '<?php echo $this->user->password?>',
								userId				: 's<?php echo $this->user->id?>',
						...
						<span id="delete_botton_<?php echo $row?>" class="delete"  onclick="deleteVideo('<?php echo $row?>', params_<?=$row?>, flashvars_<?=$row?>);"><img src="<?=  assets_url()?>images/icons/rejected.png" /></span>						
						<a href="<?php echo site_url('performer/videos/edit/' . $video->video_id)?>" class="edit right">&nbsp;</a>
						...
						<span  id="delete-<?php echo $row?>" style="visibility: hidden;" ></span>
					</div>
				<?php endforeach?>
			<?php endif?>
		</div>
