 ...
 */
Class Messenger_controller extends  MY_Studio_Edit{
	...
	function inbox(){
		...
		$data['messages'] 		= $this->messages->get_all_received_by_user_id($this->performer->id, 'performer', FALSE, FALSE, FALSE);
		$data['unread_number'] 	= $this->messages->get_all_received_by_user_id($this->performer->id, 'performer', FALSE, FALSE, TRUE, TRUE);
			
		
		$data['folder'] = 'inbox';
		
		$data['_performer_menu']		= TRUE;
		$data['page'] 					= 'performers/messenger/messenger_inbox';
		$data['description'] 			= SETTINGS_SITE_DESCRIPTION;
		$data['keywords'] 				= SETTINGS_SITE_KEYWORDS;
		$data['pageTitle'] 				= $this->performer->nickname.'\'s '.lang('Messenger').' - '.SETTINGS_SITE_TITLE;
	...
	function mail_list(){
		...
		if($folder == 'inbox'){
			...
			$data['messages'] 	= $this->messages->get_all_sent_by_user_id($this->performer->id, 'performer', FALSE, FALSE, FALSE);
			
		}elseif($folder == 'trash'){
			
			$data['messages'] 	= $this->messages->get_all_trashed_by_user_id($this->performer->id, 'performer', FALSE, FALSE, FALSE);
		...
		if(is_array($data['messages']) && count($data['messages']) > 0){
			$json_response['list'] = $this->load->view('performers/messenger/messenger_list', $data, TRUE);		
	...
	*/
	function mail(){
		...
		if( $this->form_validation->run() === FALSE ){
			...
			if ($this->performer->id == $data['message']->to_id && 'performer' == $data['message']->to_type) {
				...
				$json_response['unread_number'] = $this->messages->get_all_received_by_user_id($this->performer->id, 'performer', FALSE, FALSE, TRUE, TRUE);
				//incarcam view-ul pentru citirea mesajului
	
				$json_response['mail'] = $this->load->view('performers/messenger/messenger_mail', $data, TRUE);
				$json_response['success'] = TRUE;
				die(json_encode($json_response));
	...
	function send_mail(){
		...
		$this->form_validation->set_rules('subject',	lang('subject'),	'trim|required|strip_tags|purify');
	...
	*/	
	function move_to_trash(){
		...
		if( $this->form_validation->run() === FALSE ){
			...
			if (($this->performer->id == $message->to_id && 'performer' == $message->to_type) //from inbox or trash
			|| ($this->performer->id == $message->from_id && 'performer' == $message->from_type)){ //from sent
				if($folder == 'inbox'){
				...
				$this->messages->update_message($message_id, $updates);
