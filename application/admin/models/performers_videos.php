...
class Performers_videos extends CI_Model{
	...
	function delete_one_by_id($id){
		$this->db->where('id',$id)->delete($this->performers_videos);
	}
	
	// -----------------------------------------------------------------------------------------		
	/**
	 * Sterge un video dupa $fms_id
	 * @param $id
	 */	
	function delete_by_fms_id($fms_id){
		$this->db->where('fms_id',$fms_id)->delete($this->performers_videos);
	}
}