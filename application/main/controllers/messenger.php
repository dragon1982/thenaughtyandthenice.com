...
class Messenger_controller extends MY_Users{
	...
	function inbox(){
		...
		$data['messages'] 	= $this->messages->get_all_received_by_user_id($this->user->id, 'user', FALSE, FALSE, FALSE);
		$data['unread_number'] = $this->messages->get_all_received_by_user_id($this->user->id, 'user', FALSE, FALSE, TRUE, TRUE);
	...
	function mail_list(){
		...
		if($folder == 'inbox'){
			...
			$data['messages'] 	= $this->messages->get_all_sent_by_user_id($this->user->id, 'user', FALSE, FALSE, FALSE);
			
		}elseif($folder == 'trash'){
			
			$data['messages'] 	= $this->messages->get_all_trashed_by_user_id($this->user->id, 'user', FALSE, FALSE, FALSE);
	...
	function mail(){
		...
		if( $this->form_validation->run() === FALSE ){
			...
			if ($this->user->id == $data['message']->to_id && 'user' == $data['message']->to_type) {
				...
				$json_response['unread_number'] = $this->messages->get_all_received_by_user_id($this->user->id, 'user', FALSE, FALSE, TRUE, TRUE);
			...
			} elseif ($this->user->id == $data['message']->from_id && 'user' == $data['message']->from_type) {
	...
	function send_mail(){		
		...
		if( $this->form_validation->run() === FALSE ){
			$jason_response['success'] = FALSE;
			$json_response['error'] = '<span class="no_messages">'. validation_errors('<div>','</div>') .'.</span>';
			die(json_encode($jason_response));				
		}
		else
		{			
			...
			{
				$jason_response['success'] = TRUE;
				$jason_response['error'] = lang('Message sent!');
				die(json_encode($jason_response));
			} else {
				$jason_response['success'] = FALSE;
				$jason_response['error'] = lang('Error sending message!');
				die(json_encode($jason_response));
			}	
		}				
	}
	...
	function move_to_trash(){		
		...
		if( $this->form_validation->run() === FALSE ){
			...
			if (
				($this->user->id == $message->to_id && 'user' == $message->to_type) //from inbox or trash
					|| 
				($this->user->id == $message->from_id && 'user' == $message->from_type)//from sent
			){ 
				if($folder == 'inbox'){
					$updates = array('trashed_by_recipient' => 1);
					$jason_response['error'] = lang('Message was moved to trash!');
				}elseif($folder == 'sent'){
					$updates = array('deleted_by_sender' => 1);
					$jason_response['error'] = lang('Message was deleted!');
				}else{
