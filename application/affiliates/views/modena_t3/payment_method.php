			...
			<?php foreach($this->payment_method_list as $payment_method):?>
				...
				<div id="payment_method_<?php echo $payment_method->id?>" class="methods" style="display:none">
					...
					<div style="width:960px;">
						...
						<?php endif?>
						<span generated="true" htmlfor="rls_amount_<?php echo $payment_method->id?>" class="error message"><?php echo (form_error('rls_amount'. '_' . $payment_method->id))? form_error('rls_amount'. '_' . $payment_method->id) : sprintf(lang('Min. %s %s'),$payment_method->minim_amount , SETTINGS_SHOWN_CURRENCY)?></span>
					</div>							
				</div>
			<?php endforeach?>		
			<br/>
